...
# collect data
av_success_probs = zeros(length(n_probs), length(maxdims))


# loop through different lengths of qubit registers
for k in 1:length(n_probs)

    println("doing n_prob = $(n_probs[k])")

    # total number of qubits in algorithm
    N = 1 + n_prec + n_probs[k]

    # save success probabilities
    #success_probs = []

    # check for different maximum bond dimensions
    for j in 1:length(maxdims)

        println("calculating maxdim = $(maxdims[j])")
        ...
        for l in 1:N_sample
            ...
            local qc = initialise_qcircuit(N, lintop, backend, maxdims[j], contmethod,
            random, randombond)
...
datafile = zeros(length(n_probs)+3, length(maxdims))

datafile[1, 1] = n_prec
datafile[1, 2] = N_sample
datafile[1, 3] = length(n_probs)
datafile[2, 1:1+length(n_probs)-1] = n_probs[:]
datafile[3, :] = maxdims[:]

for i in 1:length(n_probs)
    datafile[3+i, :] = av_success_probs[i, :]
end

open("../Data/Quantum_Phase_Estimation/QPE_MPS_success_prob_n_prec_$(n_prec)_maxbond_$(maxdims[end])_samples_$(N_sample).csv", "w") do io
