...
for n_prob in n_probs
    ...
    if backend == "MPS_ITensor"
        if initialisation == "eigenstate"
            ...
            path = "../Plots/Quantum_Phase_Estimation/one_site/QPE_freq_MPS_eigenstate_nprec_$(n_prec)_nprob_$(n_prob)_maxdim_$(maxdim).png"
        else # superposition
            title = "MPS, superposition prepared, n_prec = $(n_prec), n_prob = $(n_prob)"
            path = "../Plots/Quantum_Phase_Estimation/one_site/QPE_freq_MPS_superpos_nprec_$(n_prec)_nprob_$(n_prob)_maxdim_$(maxdim).png"
        end
        make_histogram(qc, n_prec, title, path, maxdim)
    else # ED
        if initialisation == "eigenstate"
            title = "ED, eigenstate prepared, n_prec = $(n_prec), n_prob = $(n_prob)"
            path = "../Plots/Quantum_Phase_Estimation/one_site/QPE_freq_ED_eigenstate_nprec_$(n_prec)_nprob_$(n_prob).png"
        else # superposition
            title = "ED, superposition prepared, n_prec = $(n_prec), n_prob = $(n_prob)"
            path = "../Plots/Quantum_Phase_Estimation/one_site/QPE_freq_ED_superpos_nprec_$(n_prec)_nprob_$(n_prob).png"
        end
