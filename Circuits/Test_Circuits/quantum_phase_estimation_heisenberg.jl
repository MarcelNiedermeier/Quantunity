...
for n_H in N_Hamiltonian
    ...
    #H = Heisenberg_model(n_H)
    #prop = exp(1.0im*H)

    # construct diagonal matrix to check
    prop = get_diagonal_matrix(n_H)

    # find exact spectrum of propagotor
    spectrum = eigvals(prop)
    println(angle.(spectrum))
    h = histogram(angle.(spectrum), bins=2^n_prec)
    savefig(h, "../Plots/Quantum_Phase_Estimation/Heisenberg_Hamiltonian/exact_histograms/QPE_freq_exact_n_H_$(n_H)_nprec_$(n_prec)_nprob_$(n_prob).png")
    ...
    for i in (N-n_H):-1:1
        ...
        CU_general!(qc, U^n, [i], [j for j in (n_prec+n_prob+1):N])
        #global n = 2*n
        n = 2*n
    end
    ...
    if backend == "MPS_ITensor"
        ...
        path = "../Plots/Quantum_Phase_Estimation/Heisenberg_Hamiltonian/QPE_freq_MPS_n_H_$(n_H)_nprec_$(n_prec)_nprob_$(n_prob)_maxdim_$(maxdim).png"
        #else # superposition
